...
<br>
	<?php 
		...
		foreach( $tracking_items as $tracking_item ){
			...
			echo '<p style="margin: 0 0 5px;" class="shipment_heading"><strong>'.sprintf(__('Shipment %s out of %s', 'trackship-for-woocommerce'), $num , $total_trackings) .'</strong></p>';
			
			echo '<div><span>'.$tracking_item['formatted_tracking_provider'].'</span><a style="margin: 0 10px;text-decoration: none;" href='.$tracking_item['formatted_tracking_link'].'><span>'.$tracking_item['tracking_number'].'</span></a> <a class="button track-button" href='.$tracking_item['formatted_tracking_link'].'><span>'.__('Track','trackship-for-woocommerce').'</span></a> </div>'; ?>
			
			<div style="margin:20px 0;">
				<strong style="border-bottom:1px solid #e0e0e0;    display: block;padding-bottom: 5px;"><?php echo $shipping_items_heading; ?></strong>
							...
							foreach( $tracking_item['products_list'] as $products_list ){								
								...
								<tr>
									...
									<td class="td" style="text-align:<?php echo esc_attr( $text_align ); ?>; vertical-align: middle; font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif; word-wrap:break-word;border-left:0;border:0;border-bottom:1px solid #e0e0e0;padding: 12px 5px;">
										<?php
										// Product name.
										echo wp_kses_post( $product->get_name() ); echo ' x '; echo $products_list->qty; ?>
									</td>	
								</tr>	
								<?php
							}
