...
class WC_Trackship_Actions {
	...
	public function filter_orders_by_shipment_status(){
		...
		if ( 'shop_order' === $typenow ) {
			...
			<select name="_shop_order_shipment_status" id="dropdown_shop_order_shipment_status">
				<option value=""><?php _e( 'Filter by shipment status', 'trackship-for-woocommerce' ); ?></option>
				...
				if( $tracking_id == $tracking_item['tracking_id'] ){
					if( isset( $shipment_status[$key] )){
                        ...
                        <div class="ast-shipment-status-div temp-pending_trackship" style="display:none;">	
                            <span class="open_tracking_details ast-shipment-status shipment-pending_trackship" data-orderid="<?php echo $order_id; ?>" data-tracking_id="<?php echo $tracking_id; ?>" >
                            	<span class="shipment-icon icon-pending_trackship">
                                	<strong><?php echo apply_filters( "trackship_status_filter",'pending_trackship' ); ?></strong>
                                </span>
							</span>
						</div>
					<?php }
	...
	*/
	public function trigger_tracking_email( $order_id, $old_status, $new_status, $tracking_item, $shipment_status ){
		...
		if( $old_status != $new_status){			
			...
			} elseif($new_status == 'failure' || $new_status == 'in_transit' || $new_status == 'on_hold' || $new_status == 'out_for_delivery' || $new_status == 'available_for_pickup' || $new_status == 'return_to_sender' || $new_status == 'exception' ){
	...
	*/
	public function ast_add_dashboard_widgets() {
		//amcharts js	
		wp_enqueue_script( 'amcharts', trackship_for_woocommerce()->plugin_dir_url() . 'assets/js/amcharts/amcharts.js' );
		wp_enqueue_script( 'amcharts-light-theme', trackship_for_woocommerce()->plugin_dir_url() . 'assets/js/amcharts/light.js' );
		wp_enqueue_script( 'amcharts-serial', trackship_for_woocommerce()->plugin_dir_url() . 'assets/js/amcharts/serial.js' );		
	...
	*/
	public function dashboard_widget_function( $post, $callback_args ) {				
		...
		foreach ( $tracking_item_merge as $key => $item ) {			
			$tracking_provider = $wpdb->get_var( "SELECT provider_name FROM {$wpdb->get_blog_prefix(0)}woo_shippment_provider WHERE ts_slug = '".$item['tracking_provider']."'" );
		...
		<div class="ast-dashborad-widget">			
			...
			<label for="tab_s_providers" class="widget_tab_label first_label"><?php _e('Shipment providers', 'trackship-for-woocommerce'); ?></label>
			
			<input id="tab_s_status" type="radio" name="tabs" class="widget_tab_input">
			<label for="tab_s_status" class="widget_tab_label"><?php _e('Shipment status', 'trackship-for-woocommerce'); ?></label>
			
			<input id="tab_t_issues" type="radio" name="tabs" class="widget_tab_input">
			<label for="tab_t_issues" class="widget_tab_label"><?php _e('Tracking issues', 'trackship-for-woocommerce'); ?></label>
			...
			<section id="content_s_providers" class="widget_tab_section">
				...
				<?php } else{ ?>
					<p style="padding: 8px 12px;"><?php _e('data not available.', 'trackship-for-woocommerce'); ?></p>
				<?php } ?>
			</section>	
		...
		<div class="widget_footer">	
			<a class="" href="https://trackship.info/my-account/analytics/" target="blank"><?php _e( 'View more on TrackShip','trackship-for-woocommerce' ); ?></a>
		</div>
	<?php }
	...
	*/
	function check_tracking_exist( $bool, $order ){
	...
	*/
	function check_order_status( $bool, $order ){
	...
	*/
	function schedule_when_order_status_changed( $order_id, $old_status, $new_status ) {
		$this->schedule_trackship_trigger( $order_id );
	}
	...
	*/
	function schedule_trackship_trigger( $order_id ){
		$order = wc_get_order( $order_id );
	...
	function set_temp_pending( $order_id ) {
		...
		foreach ( $tracking_items as $key => $tracking_item ) {
			...
			if ( isset( $shipment_statuses[$key]['status'] ) && $shipment_statuses[$key]['status'] == 'delivered' ) {
				continue;
			}
			
			$shipment_statuses[$key]['pending_status'] = 'pending_trackship';
			$shipment_statuses[$key]['status_date'] = date( 'y-m-d' );
		}
		//echo '<pre>';print_r($shipment_statuses);echo '</pre>';
	...
	*/
	function trigger_trackship_apicall( $order_id ){
	...
	* param $order_id
	*/	
	function schedule_while_adding_tracking( $order_id ){
		$this->schedule_trackship_trigger( $order_id );
	}
	...
	*/
	public function get_custom_order_number($order_id){
		...
		if ( is_plugin_active( 'custom-order-numbers-for-woocommerce/custom-order-numbers-for-woocommerce.php' ) ) {
			...
			if(!empty($custom_order_number))return $custom_order_number;				
		}		
		...
		if ( is_plugin_active( 'wp-lister/wp-lister.php' ) || is_plugin_active( 'wp-lister-for-ebay/wp-lister.php' )) {
			...
			if(empty($custom_order_number))$custom_order_number = get_post_meta( $order_id, '_ebay_order_id', true );
			if(!empty($custom_order_number))return $custom_order_number;	
		}	
		...
		if ( is_array( $shipment_statuses ) ) {
			if ( $formatted ){
				...
				foreach ( $shipment_statuses as &$item ) {
					...
					if( $item[ 'status' ] != 'carrier_unsupported' ) {
	...
	public function get_trackship_key() {
		return $wc_ast_api_key = get_option( 'wc_ast_api_key' );
	}
