...
if ( $completed_order_with_tracking > 0 || $completed_order_with_zero_balance > 0 || $completed_order_with_do_connection > 0 ) {
    ...
    <div class="trackship-notice">
    	<p><?php echo sprintf( __( 'We detected %s Shipped orders from the last 30 days that were not sent to TrackShip, you can bulk send them to TrackShip', 'trackship-for-woocommerce'), $total_orders ); ?></p>
			...
			<div class="ts-widget-content ">
				<div class="ts-widget-row">
					<div class="ts-widget__section ts-widget-rborder ts-widget-bborder">
                        ...
                        <span>(<?php echo $tracking_analytics['total_orders']; ?> <?php esc_html_e( 'Orders', 'woocommerce' ); ?>)</span>
                    </div>
                    <div class="ts-widget__section ts-widget-bborder">
                        <h3><?php esc_html_e( 'Avg Shipment Length', 'trackship-for-woocommerce' ); ?></h3>
                        <span class="ts-widget-analytics-number"><?php echo round((int)$tracking_analytics['avg_shipment_length']); ?></span>
                ...
                <div class="ts-widget-row">
                    <div class="ts-widget__section">
                        <div style="float:right;">
                            <a href="https://trackship.info/my-account/?utm_source=wpadmin&utm_medium=sidebar&utm_campaign=upgrade" class="button-primary btn_large btn_outline" target="_blank" ><?php _e( 'Account Dashboard', 'trackship-for-woocommerce' ); ?></a>
                        </div>
                        <div>
                            <p>
                                <span>
                                    <?php esc_html_e( 'Subscription ', 'trackship-for-woocommerce' ); ?>:
                                    <?php if(isset($plan_data->subscription_plan))echo $plan_data->subscription_plan; ?>
                                </span>
                            </p>
                            <p><?php esc_html_e( 'Trackers Balance', 'trackship-for-woocommerce' ); ?>: <?php echo get_option('trackers_balance'); ?></p>
                        </div>
                    </div>
                </div>
            </div>
