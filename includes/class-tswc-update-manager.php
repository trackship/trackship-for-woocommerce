...
class TSWC_Update_Manager {
	...
	*/
	...
	public function addUpgradeMessageLink() {		
		...
		if ( trackship_for_woocommerce()->license->get_license_status() )return;
		
		$url = admin_url( 'admin.php?page=trackship-for-woocommerce&tab=license' );
		
		echo sprintf( ' ' . esc_html__( 'To receive automatic updates license activation is required. Please visit %ssettings%s to activate your TrackShip for WooCommerce.', 'trackship-for-woocommerce' ), '<a href="' . esc_url( $url ) . '" target="_blank">', '</a>' );
