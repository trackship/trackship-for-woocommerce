...
class WC_TrackShip_Late_Shipments {
	...
	*/
	public function init(){
		...
		if(!$wcast_enable_late_shipments_email || !$wc_ast_api_key)return;			
	...
	*/
	...
	public function late_shippment_email_trigger( $order_id, $order, $tracker, $tracking_number ){
		...
		$email_content = sprintf( __( 'This following order were shipped %s days ago and are not delivered.', 'trackship-for-woocommerce' ), $this->get_shipment_length( $tracker ) );
	...
	*/
	...
	function late_shipment_pre_text( $tracking_item, $order_id ) {
		?>
        <p><?php echo sprintf( __( 'Order #%d shipped on %s:', 'trackship-for-woocommerce' ), $order_id, date_i18n( get_option('date_format'), $tracking_item['date_shipped'] ) ); ?></p>
        <?php
	}
	...
	function test123_cb(){
		...
		$tracking_number = '1Z2174RY6890314868';
		
		$shipment_status = trackship_for_woocommerce()->actions->get_shipment_status( $order_id );
		$tracker = $shipment_status[0];
		
		$this->late_shippment_email_trigger( $order_id, $order, $tracker, $tracking_number );
	}
}
