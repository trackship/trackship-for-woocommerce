	...
	*/
	...
	public function send_on_hold_shipments_email() {
		...
		if ( in_array( get_option( 'user_plan' ), array( 'Free Trial', 'Free 50', 'No active plan' ) ) ) {
			...
			$context = array( 'source' => 'trackship_on_hold_shipments_email' );
		...
		$on_hold_ship_day = get_trackship_settings( 'on_hold_shipments_days', 5);
		$day = $on_hold_ship_day - 1;
		...
		foreach ( $total_order as $key => $orders ) {
			if ( in_array( 1, $email_send ) ) {
				...
				$wpdb->update( $woo_trackship_shipment, array( 'on_hold_email' => 1 ), $where );
			}
		}
		exit;
	}
